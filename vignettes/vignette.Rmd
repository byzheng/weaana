...
Function “sites2KML” convert site positions to KML file. The station names could not be output with “name = FALSE”.
```{r}
#sites2KML(records, file = "sites.kml", name = FALSE)
```

Function “plotSites” shows positions of all weather stations in a map (Figure 4.1).
```{r}
plotSites(records, lines = ozRegion(), sitename = FALSE, col = "red", pch = 24)
```

The default map is Australia map from oz package. The map can be specified by parameter “lines” which is a list of latitude and longitude of all lines. The site name, colour and symbol style could be set by other parameters.
...
wcal(records, mov.avg2 = "mov.avg(avgt, k = 5, shift = \"begin\")")
```
